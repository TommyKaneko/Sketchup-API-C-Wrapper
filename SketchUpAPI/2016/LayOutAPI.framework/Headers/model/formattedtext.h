...
*/
typedef enum {
  LOFormattedTextGrowMode_Unbounded = 0, ///< The text bounds will automatically adjust itself to the size of the current text, and lines will not wrap.
  LOFormattedTextGrowMode_Bounded,       ///< The text bounds is set explicitly. Lines that go beyond the right side of the bounds will wrap, and may not be visible if wrapping causes the text to go beyond the bottom of the bounds.
...
- \ref SU_ERROR_NULL_POINTER_INPUT if anchor_point is NULL
- \ref SU_ERROR_OUT_OF_RANGE if anchor_type is not a valid value
- \ref SU_ERROR_SERIALIZATION if the file referred to by path is not valid
...
- \ref SU_ERROR_GENERIC if page refers to a page in a different document from
  rtf_text.
*/
...
@brief Gets the style of a formatted text object at the specified plain text
       character index.
@param[in]  text            The formatted text object.
@param[in]  character_index The index of the character position to get the
                            style of. Must be greater than or equal to 0, and
                            less than the length returned by
...
/**
@brief Gets the style and length of the style run at the specified plain text
       character index.
@param[in]  text            The formatted text object.
@param[in]  character_index The index of the character position to get the
                            style of. Must be greater than or equal to 0, and
                            ...
                            \ref LOFormattedTextGetPlainTextLength.
@param[out] style           The style object.
@param[out] length_of_run   The number of characters starting at
                            character_index for which the style applies.
@return
- \ref SU_ERROR_NONE on success
- \ref SU_ERROR_INVALID_INPUT if text does not refer to a valid object
- \ref SU_ERROR_INVALID_OUTPUT if style does not refer to a valid object
- \ref SU_ERROR_OUT_OF_RANGE if character_index is out of range of the plain
  text length
- \ref SU_ERROR_NULL_POINTER_OUTPUT if length_of_run is NULL
*/
LO_RESULT LOFormattedTextGetStyleRunAtCharacter(LOFormattedTextRef text,
                                                size_t character_index,
                                                LOStyleRef style,
                                                size_t* length_of_run);

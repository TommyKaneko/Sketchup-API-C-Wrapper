...
LO_RESULT LOLinearDimensionCreateDimensionTextCopy(
    LOLinearDimensionRef dimension,
...
@brief Sets the scale being used for the dimension length text. This will also
       make the scale not use auto-scale. LayOut only allows for a finite set
       of scale values, so it will set the scale to the closest valid setting.
       See the "Scale" section of LayOut's "Document Setup" dialog for a
       reference of the available scales.
@param[in] dimension The linear dimension object.
@param[in] scale     The new scale to use.
@return
- \ref SU_ERROR_NONE on success
- \ref SU_ERROR_INVALID_INPUT if dimension does not refer to a valid object
- \ref SU_ERROR_OUT_OF_RANGE if scale is not positive
- \ref SU_ERROR_LAYER_LOCKED if dimension is on a locked layer
*/
...
- \ref SU_ERROR_NULL_POINTER_OUTPUT if point is NULL
*/
LO_RESULT LOLinearDimensionGetEndConnectionPoint(
    LOLinearDimensionRef dimension,
    LOPoint2D* point);
...
- \ref SU_ERROR_LAYER_LOCKED if dimension is on a locked layer
*/
LO_RESULT LOLinearDimensionSetEndConnectionPoint(
    LOLinearDimensionRef dimension,
    LOPoint2D* point);
...
- \ref SU_ERROR_LAYER_LOCKED if dimension is on a locked layer
*/
LO_RESULT LOLinearDimensionSetStartOffsetLength(
    LOLinearDimensionRef dimension,
    double length);
...
- \ref SU_ERROR_LAYER_LOCKED if dimension is on a locked layer
*/
LO_RESULT LOLinearDimensionSetEndOffsetLength(
    LOLinearDimensionRef dimension,
    double length);
